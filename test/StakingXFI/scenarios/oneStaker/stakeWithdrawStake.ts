...
export const stakeWithdrawStake = async function () {
    ...
    describe("Calling Reward Notify ", function (){
        it("stake BEFORE notify, withdraw X tokens several times in the middle of reward duration", async function () {
            ...
            expect(await rewardToken.balanceOf(staker) - stakerInitRewardBalance).to.approximately(balanceChange, 10000n);
        ...
        it("stake AFTER notify, withdraw X tokens several times in the middle of reward duration", async function () {
            ...
            expect(await rewardToken.balanceOf(staker)).to.approximately(tokenRewardAmount - (tokenRewardAmount / tokenDuration), 1000n);
        })
    ...
    describe("Calling Native Notify", function (){
        ...
        it("stake AFTER notify, withdraw X tokens several times in the middle of reward duration", async function () {
            ...
            expect(await staking.balanceSTOf(staker)).to.approximately(tokenRewardAmount - (tokenRewardAmount / nativeDuration), 10000n);
        })
    })
}